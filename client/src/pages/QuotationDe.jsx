...
import AddCustomer from "../components/AddCustomer"; // adjust path if needed
import {
  quotationData,
  productList,
  customersDummy,
...
/* ---------- React-Select decorations (with images) ---------- */
...
const QuotationDetail = () => {
  ...
  /* Customer state (so we can add new customers locally) */
  
  const [customers, setCustomers] = useState(customersDummy || []);
  const [isAddCustomerOpen, setIsAddCustomerOpen] = useState(false);

  /* Find summary row from listing data */
  const listRow = useMemo(
    () => quotationData.find((q) => q.quotationNo === quotationNo),
    [quotationNo]
  );

  /* Build detailed object from listRow + dummy defaults */
  const detailed = useMemo(() => {
    if (!listRow) return null;

    return {
      quotationNo: listRow.quotationNo,
      series: "SAL-QTN-YYYY",
      date: listRow.createdDate,
      validTill: listRow.expiryDate,
      deliveryDate: listRow.deliveryDate,
      status: listRow.status,
      orderType: listRow.type,
  ...
  /* ---------- Seed state from detailed once it's available ---------- */
  useEffect(() => {
    if (!detailed) return;

    setForm((prev) => ({
      ...prev,
      series: detailed.quotationNo || detailed.series || prev.series,
      createdDate: detailed.date || listRow?.createdDate || prev.createdDate,
      expiryDate: detailed.validTill || listRow?.expiryDate || prev.expiryDate,
      deliveryDate:
        detailed.deliveryDate || listRow?.deliveryDate || prev.deliveryDate,
      orderType: detailed.orderType || listRow?.type || prev.orderType,
    ...
    // Ensure the detailed customer is present in our local customers list
    ...
    // Seed customerAddressesDB for this customer so selects show something
    if (detailed.customer && detailed.addresses) {
      if (!customerAddressesDB[detailed.customer]) {
        customerAddressesDB[detailed.customer] = {
          billing: [],
          shipping: [],
          company: [],
          contactPersons: [],
      ...
      const db = customerAddressesDB[detailed.customer];
      if (
    ...
    return (
      <DashboardLayout>
        ...
        <div className="text-sm text-gray-400 mb-4">
          ...
          / <span className="font-semibold text-gray-700">View Quotation</span>
        </div>
        <div className="bg-white rounded-xl border border-red-200 text-red-600 p-6">
          Quotation <span className="font-mono">{quotationNo}</span> not
          found.
          <button
            onClick={() => navigate("/sales/quotation")}
            className="ml-3 px-3 py-1.5 rounded-md text-sm bg-rose-600 text-white hover:bg-rose-700"
          >
            Back to list
          </button>
        </div>
  ...
  /* Derived: addresses for the selected customer */
  const selectedCustomerAddresses =
    customerAddressesDB[form.customer] || {
      billing: [],
      shipping: [],
      company: [],
      contactPersons: [],
  ...
  /* ---------- Inline Add Address save handler ---------- */
  const handleAddAddressSave = (newValue) => {
    ...
    if (!customerAddressesDB[form.customer]) {
      customerAddressesDB[form.customer] = {
        billing: [],
        shipping: [],
        company: [],
    ...
    customerAddressesDB[form.customer][key].push(newValue);
  ...
  const totalMargin = items.reduce((s, it) => {
    const totalBuy =
      Number(it.product.buyingRate || 0) * Number(it.qty || 0);
    const margin = Number(it.rate || 0) - totalBuy;
    return s + margin;
  }, 0);

  /* ---------- Taxes & Charges behaviour ---------- */
  useEffect(() => {
    setTaxRows((prev) =>
      prev.map((row) => {
  ...
  const handleDownloadQuotation = () => {
    ...
    a.href = url;
    a.download = `${form.series || "quotation"}.json`;
    a.click();
    URL.revokeObjectURL(url);
    toast.info("Downloaded current quotation data (JSON).");
  };
  ...
  const handleSaveInvoice = () => {
    ...
    if (
      ...
      toast.error("Expiry Date cannot be before Created Date.");
      return;
    }
    ...
    openConfirm({
      ...
      onConfirm: () => {
        ...
        try {
          localStorage.setItem(`quotation:${orderId}`, JSON.stringify(payload));
        } catch (e) {
        ...
        toast.success("Quotation saved. You can now submit it.");
      },
    });
  };
  ...
  const handleSubmitQuotation = () => {
    ...
    openConfirm({
      ...
      onConfirm: () => {
        setIsSubmitted(true);
        toast.success("Quotation submitted. You can now create a Sales Order.");
      },
    });
  };
  ...
  const goToSalesOrder = () => {
    ...
    const payload = buildCurrentPayload(savedOrderId);
    navigate(`/sales/sales-order/${savedOrderId}`, { state: payload });
  };

  const handleCreateSalesOrder = () => {
    if (!isSubmitted || !savedOrderId) {
      toast.error("Please submit the quotation before creating a Sales Order.");
      return;
    }

    openConfirm({
      title: "Create Sales Order",
      message: "Create Sales Order from this quotation?",
      confirmLabel: "Create",
      onConfirm: () => {
        goToSalesOrder();
      },
  ...
  const status = detailed.status || listRow?.status || "Draft";

  /* ---------- Customer created handler (from AddCustomer popup) ---------- */
  const handleCustomerCreated = (newCustomer) => {
    const name =
      typeof newCustomer === "string" ? newCustomer : newCustomer?.name;
    if (!name) return;
    ...
    // ensure customer has an entry in address DB
    if (!customerAddressesDB[name]) {
      customerAddressesDB[name] = {
        billing: [],
        shipping: [],
        company: [],
        contactPersons: [],
    ...
    <DashboardLayout>
      ...
      <div className="text-sm text-slate-400 mb-4 flex items-center gap-1">
        ...
        <span className="font-semibold text-slate-700">
          View Quotation #{detailed.quotationNo}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-lg md:text-2xl font-bold text-slate-800 truncate max-w-[60vw]">
                {detailed.quotationNo}
              </h1>
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border border-slate-200 bg-white ${statusColor(
                  status
                )}`}
              >
                {status}
        ...
        <div className="flex flex-wrap items-center gap-2 justify-end">
          <button
            ...
            onClick={() =>
              navigate("/sales/quotation/add-quotation", {
                state: { duplicateFrom: detailed },
              })
            }
          >
            Duplicate
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs sm:text-sm rounded-md bg-slate-700 text-white hover:bg-slate-800 disabled:opacity-60"
          >
            Save
          </button>
          <button
            ...
            className="px-3 py-1.5 text-xs sm:text-sm rounded-md bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-60"
          >
            Submit
          </button>
        </div>
          ...
          <>
            ...
            <div className="mb-10">
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                {/* Left: Upload / Download / Print */}
                <div className="flex flex-wrap gap-2 items-center">
                  <input
                    id="upload-quotation-items"
                    type="file"
                    className="hidden"
                    onChange={handleUploadFile}
                  />
                  <label
                    htmlFor="upload-quotation-items"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              ...
              <div>
                ...
                <Select
                  ...
                  value={
                    form.priceList
                      ? { value: form.priceList, label: form.priceList }
                      : null
                  }
                  onChange={(opt) =>
                    setForm((f) => ({
