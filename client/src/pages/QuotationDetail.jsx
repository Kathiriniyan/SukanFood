...
const QuotationDetail = () => {
  ...
  useEffect(() => {
    ...
    if (detailed.customer && detailed.addresses) {
      const { billing, shipping, company, contactPerson } = detailed.addresses;
      setAddressesDB((prev) => {
        const existing =
          prev[detailed.customer] || {
            billing: [],
            shipping: [],
            company: [],
            contactPersons: [],
        ...
        if (billing && !updated.billing.includes(billing)) {
          updated.billing = [...updated.billing, billing];
        }
        if (shipping && !updated.shipping.includes(shipping)) {
          updated.shipping = [...updated.shipping, shipping];
        }
        if (company && !updated.company.includes(company)) {
          updated.company = [...updated.company, company];
        }
        if (contactPerson && !updated.contactPersons.includes(contactPerson)) {
          updated.contactPersons = [...updated.contactPersons, contactPerson];
        }

        return {
          ...prev,
          [detailed.customer]: updated,
        };
      });
    }
    ...
    return (
      <DashboardLayout>
        ...
        <div className="bg-white rounded-xl border border-red-200 text-red-600 p-6">
          Quotation <span className="font-mono">{quotationNo}</span> not found.
          <button
            onClick={() => navigate("/sales/quotation")}
    ...
    <DashboardLayout>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        ...
        <div className="flex flex-wrap items-center gap-2 justify-end">
          ...
          className="px-3 py-1.5  rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm shadow"
        >
          + Add Customer
        </button>
        </div>
          ...
          <>
            <div className="mb-10 bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-6 text-sm">
                ...
                {/* Customer + Add Customer button (navigates to /sales/add-customer) */}
                <div>
                  <label className="block text-gray-600 mb-1 font-medium">
              ...
              <div className="md:hidden border-t border-gray-200 px-3 py-4 bg-gray-50">
                <div className="rounded-xl bg-white shadow-sm border border-gray-200 p-3 space-y-3">
                  ...
                  <button
                    ...
                    disabled={!form.customer}
                    title={
                      !form.customer ? "Select a customer to add items" : "Add"
                    }
                  >
                    Add Item
                  </button>
                ...
                {taxRows.map((row, index) => (
                  ...
                  >
                    ...
                    <div className="px-3 py-3 grid grid-cols-2 gap-3 text-xs">
                      ...
                      <div>
                        <div className="text-gray-500 mb-1">Amount (EUR)</div>
                        <input
                          type="number"
                          step="0.01"
                          className={inputClass}
