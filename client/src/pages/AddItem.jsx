    ...
    <div className="relative">
      <input
        ...
        className={`w-full px-3 py-2 rounded-lg border text-sm outline-none transition-all ${
           disabled ? "bg-gray-100 border-gray-200 text-gray-500" : "bg-white border-gray-300 focus:ring-2 focus:ring-red-500 focus:border-red-500"
        }`}
      />
      {suffix && (
...
const AddItem = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Inventory
    openingStock: 0,
    stock: 0, // Synced with opening stock initially
    shelfLifeDays: 0,
    weightPerUnit: 0,
    weightUom: "kg",
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      // In a real app, you'd upload 'file' to a server and get a URL back.
      // Here we just store the objectUrl to simulate it working.
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      if (!formData.name || !formData.itemGroup || !formData.productType || !formData.defaultUom || !formData.standardSellingRate) {
        alert("Please fill in all required fields marked with *");
        return;
      }
    }
    
    // Logic for syncing stock
    if (currentStep === 2) {
  ...
  const handleSkipTax = () => {
    handleUpdate("taxRegion", "");
    handleUpdate("taxPercentage", 0);
    submitForm();
  };
    ...
    <DashboardLayout>
      <div className=" mx-auto pb-20">
        ...
        <div className="flex items-center gap-4 mb-8">
           <button onClick={() => navigate("/stock")} className="p-2 hover:bg-gray-100 rounded-full text-gray-600 transition">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                   ...
                   <div className="lg:col-span-2 space-y-5">
                      <div className="grid grid-cols-2 gap-4">
                         <div className="col-span-2">
                           <InputField label="Item ID (Auto-Generated)" value={formData.id} disabled={true} />
                         </div>
                         <div className="col-span-2">
                           <InputField 
                              label="Item Name" 
                 ...
                 <div className="max-w-2xl mx-auto space-y-6">
                    <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 flex items-center justify-between">
                       ...
                       <button 
                         ...
                         className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.isForWebshop ? "bg-red-600" : "bg-gray-300"}`}
                       >
                         <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formData.isForWebshop ? "translate-x-6" : "translate-x-1"}`}/>
                       </button>
                    </div>
          ...
          <div className="bg-gray-50 px-8 py-4 border-t border-gray-200 flex justify-between items-center">
             ...
             <div className="flex items-center gap-3">
               ...
               {currentStep === 4 && (
                 <button onClick={handleSkipTax} className="px-4 py-2 text-sm text-gray-500 hover:text-gray-700 font-medium">
                   Skip
                 </button>
               )}
               ...
               {currentStep < 4 ? (
                 <button 
                   onClick={handleNext}
                   className="flex items-center gap-2 px-8 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 shadow-md shadow-red-200 transition"
                 >
                 ...
                 <button 
                   onClick={submitForm}
                   className="flex items-center gap-2 px-8 py-2.5 rounded-xl bg-green-600 text-white font-medium hover:bg-green-700 shadow-md shadow-green-200 transition"
                 >
                   <FiSave /> Save Item
                 </button>
               )}
